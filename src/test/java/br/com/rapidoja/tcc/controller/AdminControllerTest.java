...
class AdminControllerTest {
    ...
    @Nested
    @DisplayName("Given get all admins")
    class GivenGetAdmins {

        @Nested
        @DisplayName("When admin list is called with success")
        class WhenAdminListIsCalledWithSuccess {
            private MvcResult result;
            private final List<AdminResponseDTO> adminResponseDTOList = AdminResponseMock.getAdminResponseList();

            @BeforeEach
            void setUp() throws Exception {
                when(adminService.findAll()).thenReturn(adminResponseDTOList);

                result = mockMvc.perform(MockMvcRequestBuilders.get("/api/admins").header("Content-Type", "application/json")).andReturn();
            }

            @Test
            @DisplayName("Then should return admin list with success")
            void thenShouldReturnAdminList() throws Exception {
                List<AdminResponseDTO> resultDto = mapper.readValue(result.getResponse().getContentAsString(), new TypeReference<List<AdminResponseDTO>>() {
                });
                assertEquals(adminResponseDTOList, resultDto);
            }
    ...
    @Nested
    @DisplayName("Given get admin by id")
    class GivenGetAdminById {
        @Nested
        @DisplayName("When admin by id is called with success")
        class WhenAdminByIdIsCalledWithSuccess {
            private MvcResult result;
            ...
            void setUp() throws Exception {
                ...
                when(adminService.findById(adminId)).thenReturn(Optional.of(adminResponseDTO));

                result = mockMvc.perform(MockMvcRequestBuilders.get("/api/admins/"+ adminId).header("Content-Type", "application/json")).andReturn();
            }
